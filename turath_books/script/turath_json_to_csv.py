...
# book_id_file = 2
# book_id = 2
# result = parse_metadata(text, book_id_file, book_id)
# print(result)
...
Slug: {section['slug']}
Level: {section['level']}
Start Page: {section['page']}
Pages: {section['pages']}
Parent ID: {section['parent_id']}
------------------------"""

def main() -> Tuple[List[Dict], Dict[str, List[int]]]:
    """Main function to process content"""
    content = [
        {'title': 'المقدمة', 'level': 1, 'page': 2},
        {'title': '- أهمية الموضوع', 'level': 2, 'page': 3},
        {'title': '- أسباب اختيار الموضوع', 'level': 2, 'page': 4},
        {'title': '- أهداف البحث', 'level': 2, 'page': 4},
        {'title': 'التمهيد', 'level': 1, 'page': 17},
        {'title': 'المبحث الأول المراد بالنوازل في الرضاع', 'level': 2, 'page': 17}
    ]
    
    # Process content
    processed_sections, section_pages = process_arabic_headings(content, book_id)
    ...
    for section_number, pages in section_pages.items():
        print(f"Section {section_number}: Pages {pages}")
    
    return processed_sections, section_pages


'''
The code follows these main steps:

Splits document into main content and footer
Processes headers marked with span tags
Extracts text between headers
Tracks references throughout the document
Organizes everything into structured lists with metadata
Each function has specific responsibilities:

get_footer(): Handles footer extraction and reference processing
get_page_structure(): Manages overall document structure and reference tracking
The approach maintains relationships between:
...
def get_structure_data_from_book_json(data, book_id_file):
    ...
    return [
        ...
        # Book metadata
        meta_data, author_page_start_data, info_data, info_long_data, non_author_data,
        
        # Volume structure
        volumes_data, volume_bound_data, volume_pair_data,
        
        # Section structure
        headings_data, page_headings_data, headings_processed_data, volume_data, section_pages_data,
...
if __name__ == "__main__":    
    # file_path = "./turath_books_json/10448.json"
    ...
    for file_path in json_files:
        ...
        # Extend master lists
        pages_data_list.extend(pages_data)
        ...
        volumes_data_list.extend(volumes_data)
        volume_bound_data_list.extend(volume_bound_data)
        volume_pair_data_list.extend(volume_pair_data)
        headings_data_list.extend(headings_data)
        page_headings_data_list.extend(page_headings_data)
        headings_processed_data_list.extend(headings_processed_data)
        volume_data_list.extend(volume_data)
        section_pages_data_list.extend(section_pages_data)
        headers_titles_data_list.extend(headers_titles_data)
        headers_text_data_list.extend(headers_text_data)
        ref_text_data_list.extend(ref_text_data)
        footer_ref_data_list.extend(footer_ref_data)

    # Create DataFrames  
    df_pages_data_list = pd.DataFrame(pages_data_list)
    df_meta_data_list = pd.DataFrame(meta_data_list)
    ...
    df_volumes_data_list = pd.DataFrame(volumes_data_list)
    df_volume_bound_data_list = pd.DataFrame(volume_bound_data_list)
    df_volume_pair_data_list = pd.DataFrame(volume_pair_data_list)
    df_headings_data_list = pd.DataFrame(headings_data_list)
    df_page_headings_data_list = pd.DataFrame(page_headings_data_list)
    df_headings_processed_data_list = pd.DataFrame(headings_processed_data_list)
    df_volume_data_list = pd.DataFrame(volume_data_list)
    df_section_pages_data_list = pd.DataFrame(section_pages_data_list)
    df_headers_titles_data_list = pd.DataFrame(headers_titles_data_list)
    df_headers_text_data_list = pd.DataFrame(headers_text_data_list)
    df_ref_text_data_list = pd.DataFrame(ref_text_data_list)
    df_footer_ref_data_list = pd.DataFrame(footer_ref_data_list)
    ...
    file_names = [
        ...
        "volumes.csv", "volume_bound.csv", "volume_pair.csv",
        "headings.csv", "page_headings.csv", "headings_processed.csv", "volume.csv", "section_pages.csv",
        "headers_titles.csv", "headers_text.csv", "ref_text.csv", "footer_ref.csv"
    ]

    data_list = [
        pages_data_list,
        meta_data_list, author_page_start_data_list, info_data_list, info_long_data_list, non_author_data_list,
        volumes_data_list, volume_bound_data_list, volume_pair_data_list,
        headings_data_list, page_headings_data_list, headings_processed_data_list, volume_data_list, section_pages_data_list,
        headers_titles_data_list, headers_text_data_list, ref_text_data_list, footer_ref_data_list
    ]
